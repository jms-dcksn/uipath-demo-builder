...
- **Will build:** discovery artifacts, Data Fabric case entity JSON, Case Management design + `sdd.md`, coordinator-owned `uipath-case-management` generation of `tasks.md` and `caseplan.json`, AI agents (coded or low-code scaffolds), Vite + React + TypeScript frontend, a narrated demo script.
...
Before starting research, ask the user directly, using Codex's user-input prompt when available, to resolve anything ambiguous. Treat these as a checklist. If the user answered all of them already in their prompt, skip — otherwise ask:
...
1. Check for a `docs/` directory in the project root. Create it if missing with the active shell. PowerShell example:
   ```powershell
...
4. Inventory the folder, then inspect each file. Cite user-provided docs as `[DOC-###]` (separate ID space from `[SRC-###]` web sources).
5. User-provided docs OUTRANK web sources when there is conflict — flag conflicts explicitly to the user.

## Workflow

1. **Scope + clarify** — state scope disclosure, ask clarifying questions with Codex's user-input prompt when available, set up `docs/`, and confirm whether the user wants full research or fast presales mode.
2. **Research the operation** using available Codex web search/fetch capabilities PLUS `docs/` contents. Follow `references/research-and-citation-rules.md`: ≥5 sources for full mode; ≥3 sources for fast presales mode. Full mode still needs ≥2 sources describing real operational workflows and ≥1 on compliance/risk where applicable. Tag web claims with `[SRC-###]` and doc claims with `[DOC-###]`. Copy `templates/use-case-research.template.md` to `builds/<demo-slug>/discovery/use-case-research.md` and fill it.
...
6. **Checkpoint with the user** — show the segment map and task matrix and ask whether to proceed to Case Management design, using Codex's user-input prompt when available. Do not hand off silently.

## Completion criteria

- Scope disclosure delivered to the user.
- All ambiguity resolved through explicit user input before research started.
...
- Back to planner, which invokes `demo-builder-data-fabric` (case entity), `demo-builder-case-management` (stage/task design → `sdd.md`), then `uipath-case-management` from the coordinator thread (`sdd.md` → `tasks.md` → `caseplan.json`).
