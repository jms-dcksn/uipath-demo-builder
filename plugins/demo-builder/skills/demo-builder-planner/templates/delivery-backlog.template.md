...
| M2 | Mock API and Flow architecture complete | Payload field map, API contracts, API project scaffold plan, node contracts, and resource discovery reviewed |  |
| M3 | Flow assets complete | Flow, agents, project-backed API Workflows, manual-start input contract, connector prerequisites, and local tool/control nodes resolved or documented |  |
...
| WI-002 | Mock API Workflow | Create API-* payloads, passthrough workflows, API project folders, project builds, and solution registration | WI-001 |  | Planned |
...
| Native API Workflow node cannot be bound before demo | Medium | Medium | Keep API Workflow project registered and use documented script placeholder with the same output contract |  |
