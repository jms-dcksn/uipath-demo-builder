...
5. **Record manual deploy work** — after `caseplan.json` is built, add Case Management publish/deploy items and any unresolved skeleton tasks to `builds/<demo-slug>/manual-completion-checklist.md`. Hand off to `uipath-platform` only when the user wants to publish now.
...
- `tasks.md` and `caseplan.json` generated by the delegated skill.
- Manual completion checklist updated with skeleton-task and publish/deploy follow-up.
